...
$("#saveServiceButton").click(function (event) {
  const name = $("#servicename").val().trim();
  const description = $("#serviceDescription").val().trim();
  const price = parseFloat($("#servicePrice").val().trim());    
...
$("#getBtn").click(function (event) {
  ...
  const description = localStorage.getItem("serviceDescription");
  const price = localStorage.getItem("servicePrice");
  $("#result").text(`Service Name: ${name}, Description: ${description}, Price: ${price}`);
});
